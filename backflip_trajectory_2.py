...
# q_start = q0.copy()
q_start = np.array([-1.38511809e-01, -1.40920565e-04,  3.04319107e-01, -3.70616031e-05,
    -3.07466379e-01, -3.41023332e-05,  9.51558944e-01, -1.59029285e-01,
    7.77087047e-01, -1.55424180e+00,  1.44677880e-01,  7.77012254e-01,
    -1.55430860e+00,  1.09009885e-01, -7.81287296e-01,  1.73552132e+00,
    -1.07544335e-01, -7.81680063e-01,  1.73624656e+00])
v_start = np.array([-5.05169683e-01,
    7.63679108e-04, -2.98449596e-01,  4.72472056e-03, -1.08212758e+00,
    -1.76357258e-04, -4.49576096e+00,  9.43868185e+00, -2.28876894e+01,
    4.19135709e+00,  9.29432061e+00, -2.24849328e+01,  2.67901364e-01,
    -3.36032038e+00,  4.38519915e+00, -2.81762917e-01, -3.36195502e+00,
    4.38753249e+00])
x0 = np.concatenate([q_start, v_start])


pin.framesForwardKinematics(robot.model, robot.data, q_start)
...
x1 = np.concatenate([q0, v0])
x_res = croco.ResidualModelState(state, x1, actuation.nu)
...
# action_1_cost_model.addCost('xReg', x_cost, 1e-1)
...
# Feet Contact & Cost
feet_contact_cost(action_1_contact_model, action_1_cost_model, hind_feet_name_list)
...
base_height_weight = 1e5
base_angle_weight = 1e3
joint_angles_weight = 1e3
joint_velocities_weight = 0 #1e0
shoulder_weight = 0
shoulder_velocities_weight = 0                

x4_activation_weights = np.array([0]*2 + [base_height_weight]*1
                                    + [base_angle_weight]*3 
...
display.displayFromSolver(solver, factor=10)
